...
(defun vt-sin (vt &key out dtype)
  (let* ((infer-dtype (or dtype (if (eq (vt-dtype vt) :float32)
				    :float32 :float64))))
    (vt-map #'sin vt :out out :dtype infer-dtype)))

(defun vt-cos (vt &key out dtype)
  (let* ((infer-dtype (or dtype (if (eq (vt-dtype vt) :float32)
				    :float32 :float64))))
    (vt-map #'cos vt :out out :dtype infer-dtype)))
...
(defun vt-atan (vt &key out dtype)
  (let* ((infer-dtype (or dtype (if (eq (vt-dtype vt) :float32)
				    :float32 :float64))))
    (vt-map #'atan vt :out out :dtype infer-dtype)))

(defun vt-sinh (vt &key out dtype)
  (let* ((infer-dtype (or dtype (if (eq (vt-dtype vt) :float32)
				    :float32 :float64))))
    (vt-map #'sinh vt :out out :dtype infer-dtype)))

(defun vt-cosh (vt &key out dtype)
  (let* ((infer-dtype (or dtype (if (eq (vt-dtype vt) :float32)
				    :float32 :float64))))
    (vt-map #'cosh vt :out out :dtype infer-dtype)))

(defun vt-tanh (vt &key out dtype)
  (let* ((infer-dtype (or dtype (if (eq (vt-dtype vt) :float32)
				    :float32 :float64))))
    (vt-map #'tanh vt :out out :dtype infer-dtype)))
...
(defun vt-asinh (vt &key out dtype)
  ...
  (let* ((infer-dtype (or dtype (if (eq (vt-dtype vt) :float32)
				    :float32 :float64))))
    (vt-map #'asinh vt :out out :dtype infer-dtype)))
...
(defun vt-hypot (t1 t2 &key out dtype)
  ...
  (let* ((infer-dtype (or dtype (if (or (eq (vt-dtype t1) :float32) 
