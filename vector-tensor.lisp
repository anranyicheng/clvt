...
(defun vt-from-array (arr &key (dtype nil))
  ...
  (let* ((shape (array-dimensions arr))
         ...
         (infer-dtype (cond
			...
			((subtypep cl-etype '(unsigned-byte 8)) :int32) ;; 有符号丢失风险
...
(defun vt-flatten-to-nested (dims data)
  ...
  (with-float-safe 
    (let ((total-size (reduce #'* dims)) ;; 总元素数
      ...
      (assert (= total-size (length data)) (data) "数据长度与维度乘积不匹配")
      (labels ((recurse (dims)
                 (if (null dims)
                     ;; 叶子节点：直接取标量并推进索引
                     (prog1 (aref data idx)
                     ...
                     (let ((n (first dims))
                       ...
                       (dotimes (i n)
                         (declare (fixnum i) (optimize (speed 3)))
                         (push (recurse sub-dims) result))
...
(defun vt-eye (rows &key (cols rows) (k 0) (value 1) (dtype :float64))
  "创建单位矩阵或矩形对角矩阵。严格对齐 NumPy 的 np.eye。
  rows: 行数。
  cols: 列数 (默认为 rows)。
  k: 对角线偏移量。正数表示主对角线之上，负数表示之下。默认为 0。
  value: 对角线填充值 (默认 1，此为非 NumPy 标准的自定义扩展)。"
  ...
  (let* ((shape (list rows cols))
         ...
         (res (%make-vt :data data
			...
			:offset 0
			:dtype dtype)))
    (declare (type (simple-array * (*)) data) (type list strides))
    (let* ((row-stride (first strides))
           ...
           (c-start (max 0 k))
           ;; 计算有效对角线长度 (防止越界)
           (diagonal-len (max 0 (min (- rows r-start) (- cols c-start)))))
      (declare (type fixnum row-stride col-stride r-start c-start diagonal-len))
      ;; 仅当存在有效对角线时才进行填充
      (when (> diagonal-len 0)
        (let ((start-offset (+ (* r-start row-stride) (* c-start col-stride))))
          (declare (type fixnum start-offset))
          ;; 沿对角线极速填充 (基于步长累加，消除循环内乘法)
  ...
  (let* ((in-shape (vt-shape tensor))
    ...
    (let* ((rows (nth (- rank 2) in-shape))
      ...
      (let* ((dtype (vt-dtype tensor))
             ...
             (in-offset (vt-offset tensor)))
        (declare (type fixnum out-idx batch-size str-r str-c in-offset))
        (dotimes (batch batch-size)
          (let ((in-ptr in-offset)
                (rem batch))
            (declare (type fixnum in-ptr rem))
            (loop for d fixnum from 0 below (- rank 2) do
              (let ((dim (the fixnum (nth d in-shp)))
                    (str (the fixnum (nth d in-strs))))
                (declare (type fixnum dim str))
                (multiple-value-bind (q r) (floor rem dim)
                  (incf in-ptr (the fixnum (* r str)))
                  (setf rem q))))
            (loop for i fixnum from 0 below diag-len
                  for src-off fixnum = (+ in-ptr
                                          (the fixnum (* (+ r-init i) str-r))
                                          (the fixnum (* (+ c-init i) str-c)))
                  do (setf (aref res-data out-idx)
                           (aref in-data src-off))
                     (incf out-idx))))
        res))))
    ...
    (let ((final-shape (vt-broadcast-shapes dest-shape src-shape)))
      ...
      (let* ((dest-data (vt-data dest))
		...
		(equal dest-shape src-shape))
          ...
          ;; 慢速路径: 非连续/需要广播 (迭代器实现)
          (t
           (let* ((rank (length dest-shape))
                  (dims (coerce dest-shape 'simple-vector))
