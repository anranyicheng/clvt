      ...
      (setf sorted-idx (sort sorted-idx
				     ...
				     (vb (aref src-data b)))
				       ...
				       ((%nan-p vb) t)
                                       (t (< va vb)))))))
      (let ((unique-vals
	      (make-array 0 :element-type elem-type :adjustable t :fill-pointer t))
            (first-idx
	      (make-array 0 :element-type '(signed-byte 64) :adjustable t :fill-pointer t))
...
(defun vt-setxor1d (t1 t2)
  (let* ((u1 (vt-unique t1)) (u2 (vt-unique t2))
    ...
    (vt-do-each (ptr val u2)
      (declare (ignore ptr))
      (unless (member val u1-set) (push val result)))
    (vt-from-sequence (vt-numpy-sort result #'<) :dtype (vt-dtype t1))))
