...
(defun vt-ref (vt &rest indices)
  "获取指定位置元素（支持负索引）。"
  (when (zerop (vt-size vt)) (error "cannot index into an empty tensor"))
  (let ((shape (vt-shape vt)) (strides (vt-strides vt)) (ptr (vt-offset vt)))
    (unless (= (length indices) (length shape))
      (error "索引数量 ~a 与维度数 ~a 不匹配" (length indices) (length shape)))
    (loop for idx in indices for dim in shape for stride in strides
          do (incf ptr (* (if (minusp idx) (+ idx dim) idx) stride)))
    (aref (vt-data vt) ptr)))
...
(defun (setf vt-ref) (value vt &rest indices)
  ...
  (let* ((shape (vt-shape vt)) (strides (vt-strides vt))
         (data (vt-data vt)) (ptr (vt-offset vt)))
    (unless (= (length indices) (length shape))
      (error "索引数量 ~a 与维度数 ~a 不匹配" (length indices) (length shape)))
    (loop for idx in indices for dim in shape for stride in strides
          do (incf ptr (* (if (minusp idx) (+ idx dim) idx) stride)))
    (setf (aref data ptr) (coerce value (vt-element-type vt)))))
