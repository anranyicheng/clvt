...
(defun vt-maximum (t1 t2 &key out dtype)
  (vt-map (lambda (a b) (cond ((vt-float-nan-p a) a) ((vt-float-nan-p b) b) (t (max a b))))
          t1 t2 :out out :dtype dtype))
(defun vt-minimum (t1 t2 &key out dtype)
  (vt-map (lambda (a b) (cond ((vt-float-nan-p a) a) ((vt-float-nan-p b) b) (t (min a b))))
          t1 t2 :out out :dtype dtype))
(defun vt-fmax (t1 t2 &key out dtype)
  (vt-map (lambda (a b) (cond ((and (vt-float-nan-p a) (vt-float-nan-p b)) a)
                              ((vt-float-nan-p a) b) ((vt-float-nan-p b) a) (t (max a b))))
          (ensure-vt t1) (ensure-vt t2) :dtype dtype :out out))
(defun vt-fmin (t1 t2 &key out dtype)
  (vt-map (lambda (a b) (cond ((and (vt-float-nan-p a) (vt-float-nan-p b)) a)
                              ((vt-float-nan-p a) b) ((vt-float-nan-p b) a) (t (min a b))))
          (ensure-vt t1) (ensure-vt t2) :dtype dtype :out out))
